...
import { downloadFile } from './filesApi.js'
import fs from 'fs'
import os from 'os'
import path from 'path'
import { ServerAPI } from './api.js'
import { downloadDependencies } from './objDependencies.js'
import { logger } from '../observability/logging.js'
import { Nullable, Scopes, wait, TIME_MS } from '@speckle/shared'
import { Knex } from 'knex'
import { getIfcDllPath, isProdEnv } from './helpers/env.js'
...
const HEALTHCHECK_FILE_PATH = path.join(os.tmpdir(), 'last_successful_query')

const TMP_INPUT_DIR = path.join(os.tmpdir(), 'file_to_import')
const TMP_FILE_PATH = path.join(TMP_INPUT_DIR, 'file')
const TMP_RESULTS_PATH = path.join(os.tmpdir(), 'import_result.json')
  ...
  try {
    ...
    if (info.fileType.toLowerCase() === 'ifc') {
      if (info.fileName.toLowerCase().endsWith('.legacyimporter.ifc')) {
        ...
        const PY =
          process.env['PYTHON_BINARY_PATH'] ||
