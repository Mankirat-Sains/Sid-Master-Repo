...
export const insertNewUploadAndNotifyFactoryV2 =
  ...
  async (upload) => {
    const file = await deps.saveUploadFile(upload)
    const fileType = file.fileType.toLocaleLowerCase()

    // File types that don't need processing (just stored as attachments)
    const attachmentFileTypes = ['pdf', 'xlsx', 'xls']

    if (attachmentFileTypes.includes(fileType)) {
      // For attachment files, mark as completed immediately without processing
      await deps.emit({
        eventName: FileuploadEvents.Updated,
        payload: {
          upload: {
            ...file,
            convertedStatus: 2, // Completed status
    ...
    const queue = deps.queues.find((q) => q.supportedFileTypes.includes(fileType))
    if (!queue) {
