...
import {
  getBlobMetadataFromStorage as getBlobMetadataFromStorageSupabase,
  getSignedUrlFactory as getSignedUrlFactorySupabase
...
const fileUploadMutations: Resolvers['FileUploadMutations'] = {
  async generateUploadUrl(_parent, args, ctx) {
    ...
    const isSupabase = isSupabaseStorage(projectStorage.public)
    const getSignedUrl = isSupabase
      ? getSignedUrlFactorySupabase({ storage: projectStorage.public as any })
      : getSignedUrlFactory({ objectStorage: projectStorage.public as any })
  ...
  async startFileImport(_parent, args, ctx) {
    ...
    const getBlobMetadata = isSupabase
      ? getBlobMetadataFromStorageSupabase({ storage: projectStorage.private as any })
      : getBlobMetadataFromStorage({ objectStorage: projectStorage.private as any })
