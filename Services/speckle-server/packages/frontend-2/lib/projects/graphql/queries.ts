...
export const projectsWithModelsQuery = graphql(`
  query ProjectsWithModels($filter: UserProjectsFilter, $cursor: String) {
    activeUser {
      projects(filter: $filter, limit: 100, cursor: $cursor) {
        items {
          id
          name
          models(limit: 100) {
...
export const projectPdfsQuery = graphql(`
  query ProjectPdfs($projectId: String!, $limit: Int, $cursor: String, $query: String) {
    project(id: $projectId) {
      id
      blobs(limit: $limit, cursor: $cursor, query: $query) {
        items {
          id
          fileName
          fileType
