...
// Strip any drive letters (C:, D:, etc.) and ensure we only use the filename
const normalizeChunkName = (chunkName: string) => {
  // Remove Windows drive letters (C:, D:, etc.) and any absolute path components
  const normalized = chunkName.replace(/^[A-Z]:[\\/]/, '').replace(/^[/\\]/, '')
  // Extract just the filename part (handles both forward and backslashes)
  return filename(normalized)
}
const buildOutputFileName = (chunkName: string) =>
  withoutLeadingSlash(
    `_nuxt/${sanitizeFilePath(normalizeChunkName(chunkName))}.[hash].js`
  )
...
export default defineNuxtConfig({
  ...
  vite: {
    ...
    server: {
      ...
      hmr: {
        // Fix WebSocket connection issues on Windows by matching the browser's host
        clientPort: 5173,
        protocol: 'ws',
        host: '127.0.0.1'
      }
