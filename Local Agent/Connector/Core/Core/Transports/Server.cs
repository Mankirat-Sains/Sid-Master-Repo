...
public sealed class ServerTransportV1 : IDisposable, ICloneable, ITransport
{
  ...
  public ServerTransportV1(Account account, string streamId, int timeoutSeconds = 600) // 10 minutes for slow servers
  {
    Account = account;
  ...
  private void Initialize(string baseUri, string streamId, string authorizationToken, int timeoutSeconds = 600) // 10 minutes for slow servers
  {
