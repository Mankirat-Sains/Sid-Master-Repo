...
{
  public const int DEFAULT_TIMEOUT_SECONDS = 600; // 10 minutes for slow servers

  public static IEnumerable<TimeSpan> DefaultDelay()
  {
    return Backoff.DecorrelatedJitterBackoffV2(TimeSpan.FromMilliseconds(500), 5); // Increased base delay for slow servers
  }
  ...
  )
  {
    using var httpClient = GetHttpProxyClient(
      new SpeckleHttpClientHandler(HttpAsyncPolicy(timeoutSeconds: 600, timeoutStrategy: TimeoutStrategy.Pessimistic)) // 10 minutes for slow servers
    );
