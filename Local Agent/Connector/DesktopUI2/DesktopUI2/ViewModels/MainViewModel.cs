...
public class MainViewModel : ViewModelBase, IScreen, IDialogHost
{
  ...
  public string TitleFull => "Sidian for " + Bindings.GetHostAppNameVersion();
  ...
  private void Init()
  {
    ...
    // Default to light theme (DarkTheme = false)
    ChangeTheme(config.DarkTheme ?? false);
  ...
  internal void ChangeTheme(bool isDark)
  {
    ...
    // Fixed: when isDark is true, use Dark theme; when false, use Light theme
    if (isDark)
    {
