...
# Load environment
# .env is in trainexcel root: backend/agents/building_code_rag.py -> backend/ -> trainexcel/
env_path = Path(__file__).parent.parent.parent / ".env"
load_dotenv(env_path)
# Initialize OpenAI client with explicit config
...
class BuildingCodeRAG:
    ...
    def __init__(self):
        # Path: agents/ -> backend/ -> Building codes/
        self.base_path = Path(__file__).parent.parent / "Building codes"
        self.codes = {}
        self.loaded = False
    ...
    def query_code(self, code_name: str, query: str, top_k: int = 3,
                   include_terms: Optional[List[str]] = None,
                   exclude_terms: Optional[List[str]] = None,
                   boost_terms: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        """
        ...
        # Format results (we will re-rank after building results list)
        results = []
        metadata = code_data["metadata"]
        
        for idx, score in similarities[: max(top_k * 3, top_k)]:  # take a larger candidate set for re-ranking
            if idx < len(metadata):
                result = {
                    "code": code_name,
                    "score": float(score),
        ...
        # Optional re-ranking: apply include/exclude/boost terms against text
        results = self._rerank(results, include_terms or [], exclude_terms or [], boost_terms or [])
        
        # Return top_k after re-ranking
        return results[:top_k]
    
    def query_all_codes(self, query: str, top_k_per_code: int = 2,
                        include_terms: Optional[List[str]] = None,
                        exclude_terms: Optional[List[str]] = None,
                        boost_terms: Optional[List[str]] = None) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        for code_name in self.codes.keys():
            code_results = self.query_code(
                code_name, query, top_k=top_k_per_code,
                include_terms=include_terms, exclude_terms=exclude_terms, boost_terms=boost_terms
            )
            if code_results:
    ...
    def _rerank(self, results: List[Dict[str, Any]],
                include_terms: List[str], exclude_terms: List[str], boost_terms: List[str]) -> List[Dict[str, Any]]:
        """Re-rank results with simple lexical constraints.
        - Exclude chunks containing any exclude_terms
        - Apply small boosts for include_terms
        - Apply larger boosts for boost_terms (e.g., selected cell symbol/label)
        """
        if not results:
            return results
        def score_with_terms(r: Dict[str, Any]) -> float:
            s = float(r.get("score", 0.0))
            text = (r.get("text") or "").lower()
            # Exclude
            for term in exclude_terms:
                if term and term.lower() in text:
                    return -1e9  # push to bottom
            # Include boosts
            ...
            for term in boost_terms:
                if term and term.lower() in text:
                    s += 0.15
            return s
        reranked = sorted(results, key=score_with_terms, reverse=True)
        # Filter out excluded ones
        reranked = [r for r in reranked if r.get("score", 0.0) > -1e8]
        return reranked
    
    def synthesize_response(self, results: List[Dict[str, Any]], user_context: str = "", 
                           material_type: str = "", sheet_type: str = "", model: str = "gpt-4o-mini") -> Optional[str]:
        """
        Synthesize a concise (~100 word) answer from RAG results using LLM.
        
        Args:
            results: List of code chunks from query_code()
            user_context: User's cell context or query details
            material_type: Material type (glulam, CLT, etc.) for filtering
            sheet_type: Sheet type (timber, concrete, etc.)
            model: LLM model to use (default: gpt-4o-mini)
            
        Returns:
            Synthesized answer string, or None if synthesis fails
        """
        if not client:
            return None
        ...
        try:
            # Use all chunks for synthesis (we retrieve top_k=5)
            snippets = "\n\n".join([f"[Page {r['page']}]\n{r['text'][:600]}" for r in results[:5]])
            
            # Extract ALL unique clause references from ALL results (not just top 3)
            all_clause_refs = []
            seen_clauses = set()
            for r in results:
                page = r.get('page', 'N/A')
                if page and page != 'N/A' and page not in seen_clauses:
                    all_clause_refs.append(f"Clause {page}")
                    seen_clauses.add(page)
            
            prompt = (
                "You are an engineering design assistant. Write a concise answer (~100 words) based on building code snippets.\n\n"
                "CRITICAL REQUIREMENTS:\n"
                "- Response must be approximately 100 words (3-5 sentences).\n"
                "- DO NOT truncate or cut off mid-sentence. Always complete your thoughts.\n"
                "- Paraphrase the code content; do NOT copy-paste large chunks.\n"
                "- List ALL relevant clause references as bullets at the end (e.g., '• Clause 7.5.12: Euler buckling load formula').\n"
                "- IMPORTANT: Include ALL clause numbers found in the snippets below, not just the most relevant ones.\n"
                "- If Euler buckling (PE), include the exact formula: PE = (π² × E05 × KSE × I) / Le².\n"
                "- If material is glulam, focus on glulam clauses; exclude CLT references.\n\n"
                f"Material: {material_type}. Sheet type: {sheet_type}.\n"
                f"User's cell context: {user_context}\n\n"
                f"Building code snippets:\n{snippets}\n\n"
                f"ALL clause references found: {', '.join(all_clause_refs) if all_clause_refs else 'See snippets above'}\n\n"
                "Write your ~100-word answer, then list ALL clause references as bullets. IMPORTANT: Include ALL clause numbers found above, do not skip any relevant clauses."
            )
            ...
            response = client.chat.completions.create(
                ...
                temperature=0.2,
                max_tokens=300,  # Enough for ~100 words + clause bullets
            )
            
            synthesized = response.choices[0].message.content.strip()
            # Remove trailing truncation markers
            if synthesized.endswith("...") or synthesized.endswith("…"):
                synthesized = synthesized.rstrip("…").rstrip("...").strip()
