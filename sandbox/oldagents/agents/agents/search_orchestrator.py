...
try:
    ...
    # Test that create_client is actually callable (supabase might be partially installed)
    if create_client is None:
        raise ImportError("create_client is None")
    HAS_SUPABASE = True
except (ImportError, ModuleNotFoundError, AttributeError) as e:
    HAS_SUPABASE = False
    create_client = None
    Client = None
    # Only print warning if it's not a missing dependency (websockets.asyncio issue)
    if "websockets.asyncio" not in str(e):
...
if HAS_LANGCHAIN and ChatOpenAI is not None:
    ...
    def make_llm(model_name: str, temperature: float = 0.1):
        try:
            if model_name.startswith("claude"):
                ...
                # Some OpenAI models (like o1, o1-mini) don't support custom temperature
                # They only support the default temperature of 1
                if model_name.startswith("o1"):
                    # O1 models don't support temperature parameter - use default
                    return ChatOpenAI(model=model_name)
                else:
                    # Try with temperature first, fallback to default if unsupported
                    try:
                        ...
                        if "temperature" in str(temp_error).lower() or "unsupported" in str(temp_error).lower():
                            log_syn.warning(f"Model {model_name} doesn't support temperature={temperature}, using default")
                            return ChatOpenAI(model=model_name)
                        else:
                            raise
        except Exception as e:
    ...
    # Use temperature 0.1 for synthesis/corrective, but skip for o1 models
...
# IMPORTANT: Also check HAS_SUPABASE to ensure create_client is available
if SUPABASE_URL and SUPABASE_KEY and HAS_SUPABASE and create_client is not None:
    ...
    elif not HAS_SUPABASE or create_client is None:
        print("WARNING: Supabase Python library not available or missing dependencies.")
        print("         Install with: pip install supabase websockets")
        print("         Or reinstall: pip install --upgrade --force-reinstall supabase")
        print("         Supabase vector stores will not be available.")
    vs_smart = None
    vs_large = None
    vs_code = None
    vs_coop = None
...
def synthesize(q: str, docs: List[Document], session_id: str = "default", stream: bool = False, project_metadata: Optional[Dict[str, Dict[str, str]]] = None, code_docs: Optional[List[Document]] = None, use_code_prompt: bool = False, coop_docs: Optional[List[Document]] = None, use_coop_prompt: bool = False, active_filters: Optional[Dict[str, Any]] = None):
    ...
    # Helper function to handle temperature errors by recreating LLM without temperature
    def safe_llm_invoke(prompt_text):
        """Invoke LLM with automatic temperature error handling"""
        global llm_synthesis
        try:
            return llm_synthesis.invoke(prompt_text)
        except Exception as e:
            error_str = str(e)
            ...
            if "temperature" in error_str.lower() and ("unsupported" in error_str.lower() or "does not support" in error_str.lower()):
                log_syn.warning(f"Model {SYNTHESIS_MODEL} doesn't support custom temperature, recreating LLM without temperature parameter")
                # Recreate LLM without temperature
                try:
                    if SYNTHESIS_MODEL.startswith("claude"):
                        from langchain_anthropic import ChatAnthropic
                        api_key = os.getenv("ANTHROPIC_API_KEY")
                        llm_synthesis = ChatAnthropic(model=SYNTHESIS_MODEL, anthropic_api_key=api_key)
                    else:
                        llm_synthesis = ChatOpenAI(model=SYNTHESIS_MODEL)
                    # Retry the call
                    return llm_synthesis.invoke(prompt_text)
                except Exception as e2:
                    log_syn.error(f"Failed to recreate LLM: {e2}")
                    raise e
            else:
                raise
    
    def safe_llm_stream(prompt_text):
        """Stream LLM with automatic temperature error handling"""
        global llm_synthesis
        try:
            return llm_synthesis.stream(prompt_text)
        except Exception as e:
            error_str = str(e)
            # Check if it's a temperature error
            if "temperature" in error_str.lower() and ("unsupported" in error_str.lower() or "does not support" in error_str.lower()):
    ...
    if stream:
        ...
        def stream_generator():
            ...
            for chunk in safe_llm_stream(formatted_prompt):
                if chunk.content:
                    if first_chunk:
        ...
        ans = safe_llm_invoke(formatted_prompt).content.strip()
