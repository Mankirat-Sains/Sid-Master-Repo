"""
Wrapper for execute_query that adds LLM-based thinking logs
...
from main import execute_query  # Import renamed function
from langchain_core.documents import Document


class QueryWrapper:
    """Wraps query execution to capture state and generate thinking logs"""
    ...
    def run_with_thinking_logs(
        ...
        """
        Wraps execute_query and adds thinking logs.
        This hooks into the graph execution to capture state at each node.
        """
        import logging
        logger = logging.getLogger(__name__)
        logger.info(f"🔄 QueryWrapper.run_with_thinking_logs: images_base64={images_base64 is not None}, count={len(images_base64) if images_base64 else 0}")
        ...
        logger.info(f"🔄 Calling execute_query with images_base64={images_base64 is not None}")
...
def execute_query_with_logs(
    question: str,
    session_id: str = "default",
    data_sources: Optional[Dict[str, bool]] = None,
    images_base64: Optional[List[str]] = None
) -> Dict:
    """Public function to call query execution with thinking logs"""
    import logging
    logger = logging.getLogger(__name__)
    logger.info(f"🔄 execute_query_with_logs called with images_base64={images_base64 is not None}, count={len(images_base64) if images_base64 else 0}")
