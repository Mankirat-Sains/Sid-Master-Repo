...
def draft_section(state: RAGState) -> dict:
    """Generate a section draft and stash in state."""
    log_query.info(
        "DOCGEN: entered draft_section | task_type=%s | doc_type=%s | section_type=%s | doc_request=%s",
        state.task_type,
        state.doc_type,
        state.section_type,
        state.doc_request,
...
def _load_csv_fallback(user_query: str) -> Dict[str, str] | None:
    ...
    if _CSV_FALLBACK_CACHE is None:
        ...
        _CSV_HAS_ROWS = False
        csv_path = Path(__file__).resolve().parents[2] / "Local Agent" / "info_retrieval" / "data" / "drafted_sections.csv"
...
class SectionDraftGenerator:
    ...
    def generate(self, doc_request: Dict[str, Any], context_path: str, output_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            log_query.error(f"DOCGEN: SectionDraftGenerator.generate failed: {exc}")
            return {
                "draft_text": f"# Document\n\nGenerated content for: {doc_request.get('title', 'Untitled') if doc_request else 'Untitled'}",
                "sections": [],
                "warnings": [f"SectionDraftGenerator fallback used: {exc}"],
                "metadata": {"fallback": True, "reason": str(exc)},
            }


# Backwards-compatible aliases for existing graph wiring and tools
node_doc_generate_section = draft_section
SectionGenerator = SectionDraftGenerator
